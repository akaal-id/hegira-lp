...
export default function EditEventPage({
  ...
  const initialEvent = initialDashboardEvents.find((e) => e.id === id) || initialDashboardEvents[0];

  const [formData, setFormData] = useState<Partial<DashboardEvent>>({
    name: initialEvent?.name || "",
    theme: initialEvent?.theme || "",
    category: initialEvent?.category || "b2c",
    status: initialEvent?.status || "Draft",
    startDate: initialEvent?.startDate || "",
    endDate: initialEvent?.endDate || "",
    timeDisplay: initialEvent?.timeDisplay || "04:00 PM - 10:00 PM WIB",
    location: initialEvent?.location || "",
    address: initialEvent?.address || "",
    coverImageUrl: initialEvent?.coverImageUrl || "",
    parkingAvailable: initialEvent?.parkingAvailable ?? true,
    totalQuota: initialEvent?.totalQuota || 1000,
    description: initialEvent?.description || "",
    termsAndConditions: initialEvent?.termsAndConditions || "",
  });

  const [amenities, setAmenities] = useState({
    parking: initialEvent?.parkingAvailable ?? true,
    ac: true,
    vip: true,
    wheelchair: true,
  });
  ...
  return (
    <form onSubmit={handleSubmit} className={styles.container}>
      ...
      <div className={styles.headerBar}>
        ...
        <h1 className={styles.headerTitle}>Edit Event</h1>
        <p className={styles.headerSubtitle}>
          Update your event details for the Hegira platform.
        </p>
      </div>
        ...
        <div className={styles.columnLeft}>
          ...
          <div className={styles.sectionCard}>
            ...
            <div className={styles.fieldGrid}>
              ...
              {/* Event Status Segmented Control */}
              <div className={`${styles.fieldGroup} ${styles.fullWidth}`}>
